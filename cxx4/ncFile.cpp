...
// constructor that opens/creates a file for parallel I/O
NcFile::NcFile(MPI_Comm& comm, MPI_Info& info, const std::string fileName, const FileMode fMode)
{
    open(comm, info, fileName, fMode);
}


// open a netCDF file for parallel I/O
void NcFile::open(MPI_Comm& comm, MPI_Info& info, const std::string fileName, const FileMode fMode)
{
    if (!nullObject)
      close();
    ...
    switch (fMode)
      {
      case NcFile::write:
        ncCheck(nc_open_par(fileName.c_str(), NC_WRITE, comm, info, &myId),__FILE__,__LINE__);
        break;
      case NcFile::read:
        ncCheck(nc_open_par(fileName.c_str(), NC_NOWRITE, comm, info, &myId),__FILE__,__LINE__);
        break;
      case NcFile::newFile:
        ncCheck(nc_create_par(fileName.c_str(), NC_NETCDF4 | NC_NOCLOBBER, comm, info, &myId),__FILE__,__LINE__);
        break;
      case NcFile::replace:
        ncCheck(nc_create_par(fileName.c_str(), NC_NETCDF4 | NC_CLOBBER, comm, info, &myId),__FILE__,__LINE__);
        break;
      }
...
// create file for paralell I/O
void NcFile::create(MPI_Comm& comm, MPI_Info& info, const std::string& fileName, int ncFileFlags)
{
    if (!nullObject)
        close();

    ncCheck(nc_create_par(fileName.c_str(), ncFileFlags, comm, info, &myId), __FILE__, __LINE__);
    g_ncid = myId;
